...
This document covers the proposed AWS infrastructure for Innovate Inc.'s web application - a Flask REST API backend with a React SPA frontend, backed by PostgreSQL. The design handles sensitive user data, starts small (hundreds of users/day), and scales to millions without re-architecting.
...
graph TB
    ...
    subgraph AWS["AWS Cloud - Production Account"]
        subgraph VPC["VPC 10.10.0.0/16 - Multi-AZ"]
            ...
            subgraph Private["Private Subnets - Application Layer"]
                subgraph EKS["Amazon EKS Cluster"]
                    direction TB
                    SysNG["System Node Group<br/>t3.small x 2-3"]
                    KarpenterCtrl["Karpenter Controller"]
                    subgraph AppPods["Application Workloads"]
                        Flask["Flask API Pods"]
                        ReactSPA["React SPA Pods"]
                    end
                    subgraph PlatformPods["Platform Workloads"]
                        ArgoCD["ArgoCD"]
                        Monitoring["Monitoring<br/>Prometheus / Grafana"]
                    end
                end
                KarpenterNodes["Karpenter-Managed Nodes<br/>Spot x86 and ARM64"]
            end

            subgraph Isolated["Isolated Subnets - Database Layer"]
                Aurora["Aurora PostgreSQL<br/>Multi-AZ"]
                AuroraReplica["Read Replica<br/>Cross-Region DR"]
            end
        end

        ECR["Amazon ECR<br/>Container Registry"]
        S3["S3<br/>Backups / State"]
        KMS["AWS KMS<br/>Encryption Keys"]
        SM["Secrets Manager"]
        CW["CloudWatch and<br/>Container Insights"]
    end
    ...
    ALB --> Flask
    ALB --> ReactSPA
    Flask -->|port 5432| Aurora
    Private -->|Outbound| NAT
    NAT --> Internet
    GH --> GHA
    GHA -->|Build and Push| ECR
    GHA -->|Update manifests| ArgoCD
    ECR -.->|Pull images| EKS
    Aurora -.->|Replication| AuroraReplica
    EKS -.->|Logs and Metrics| CW
    Aurora -.->|Encrypted| KMS
    EKS -.->|Secrets| SM
```
...
![AWS Organization](InnovateIncAWS_OU.jpg)
...
> **Note:** As Innovate Inc. grows, a third account (e.g., **Shared Services**) can be introduced for centralized logging, CI/CD tooling, and container registries. The two-account structure is the right starting point for a small team — it keeps overhead low while still providing the isolation benefits of multi-account architecture.

**Security baseline applied to both accounts:**

- IAM least-privilege policies scoped per role (developers, operators, CI/CD)
- MFA enforced for all IAM users and root accounts
...
![VPC Architecture](VPC_Networking.jpg)
...
![EKS and Database Architecture](EKS_RDS.jpg)
...
- **Ecosystem maturity** — native integrations with IAM (IRSA/Pod Identity), ALB Ingress Controller, and VPC CNI
- **Karpenter support** — purpose-built for EKS, enabling intelligent node provisioning
- **Compliance** — SOC 2, ISO 27001, HIPAA-eligible out of the box
...
```mermaid
flowchart LR
    A["Developer<br/>Push to main"] --> B["GitHub Actions<br/>Build and Test"]
    B --> C["Docker Build<br/>Multi-stage"]
    C --> D["ECR Push and<br/>Vuln Scan"]
    D --> G["Update Helm<br/>Values in Git"]
    G --> H["ArgoCD<br/>Detects Change"]
    H --> I["Sync to<br/>EKS Cluster"]
    I --> J["Health Check<br/>and Validation"]
    J -->|Fail| K["Auto<br/>Rollback"]
    J -->|Pass| L["Live in<br/>Production"]
```
...
- **Network Policies** (Calico/Cilium) restrict pod-to-pod traffic to only allowed communication paths
...
- IMDSv2 required on all nodes (`httpTokens: required`) to prevent SSRF-based credential theft
...
### Estimated Monthly Cost at Launch (Low Traffic)

| Service | Estimated Cost |
|---------|---------------|
| EKS control plane | ~$73 |
| EC2 (2× t3.small system nodes) | ~$30 |
| Aurora PostgreSQL (db.t4g.medium) | ~$60 |
| NAT Gateway + data transfer | ~$35 |
| ECR, S3, CloudWatch | ~$15 |
| **Total** | **~$213/month** |

As traffic grows, autoscaling adds capacity only as needed — costs scale linearly with actual usage rather than requiring upfront over-provisioning.
...
| **Logging** | Fluent Bit → CloudWatch Logs | Centralized log aggregation from all pods and nodes |
...
|-------|-----------|----------|
| High API error rate | 5xx errors > 1% of requests over 5 minutes | Critical |
| Pod crash loop | Pod restarts > 3 in 10 minutes | Critical |
| High CPU/memory | Node or pod utilization > 85% sustained | Warning |
| Database connections exhausted | Active connections > 80% of max | Critical |
| Unauthorized access attempts | CloudTrail anomaly detection | Critical |
| Budget threshold | Monthly spend exceeds 80% of budget | Warning |
